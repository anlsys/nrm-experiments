...
```{r, echo=FALSE, fig.cap="", dev="pdf", fig.width=11, fig.height=3}
  ggplot(data=df[which((df$name=="pcap250/250" |df$name=="pcap70/70") & df$variable=="sensor-RaplKey (PackageID 0)" ) ,],
...
```{r, echo=FALSE, fig.cap="", dev="pdf", fig.width=11, fig.height=3}
  ggplot(data=df[which((df$name=="pcap250/250" |df$name=="pcap70/70") & df$variable=="sensor-RaplKey (PackageID 1)" ) ,],
         aes(time, as.numeric(as.character(value)),group=name,color=name) )+
    geom_point() +
    geom_line() +
    facet_grid(rows = vars(iteration), scales="free_y") +
