...
def do_workload(baseActions, daemonCfg, workloads, actuations=[]):
    with nrm.nrmd(daemonCfg) as d:
        print("Starting the workloads")
        for w in workloads:
            d.run(**w)
                ...
                if "bandit" in controller.keys():
                    ...
                    if "referenceMeasurementDecision" in meta.keys():
                        ...
                        for value in meta["innerDecision"]["objectives"]:
                            ...
                            counter = counter + 1
                        counter = 0
                        for value in meta["innerDecision"][
                            "reportNormalizedObjectives"
                        ]:
                            print(value)
                            history[
                                "reportNormalizedObjectives(weight)-" + str(counter)
                            ].append((t, value[0]))
                            history[
                                "reportNormalizedObjectives(value)-" + str(counter)
                            ].append((t, value[1]))
                            counter = counter + 1
                        counter = 0
                        for value in meta["innerDecision"][
                        ...
                        for value in meta["innerDecision"]["reportEvaluatedObjectives"]:
                            interval = value[2]["i"]
                            history[
                                "reportEvaluatedObjectives(weight)-" + str(counter)
                            ].append((t, value[0]))
                            history[
                                "reportEvaluatedObjectives(measurement)-" + str(counter)
                            ].append((t, value[1]))
                            history[
                                "reportEvaluatedObjectives(inf)-" + str(counter)
                            ].append((t, interval[0]))
                            history[
                                "reportEvaluatedObjectives(sup)-" + str(counter)
                            ].append((t, interval[1]))
                            counter = counter + 1
                    ...
                    if "lagrange" in bandit.keys():
                        for arm in bandit["lagrange"]["lagrange"]["weights"]:
                            ...
                            history[
                                armShorthandDescription(arm["action"])
                                + "-cumulativeLoss"
                ...
                for arm, stats in controller["armstats"]:
                    ...
                    for name, value in stats[4]:
                        ...
                        history[
                            ...
                            + name
                            + "-"
                            + armShorthandDescription(arm)
                        ].append((t, value))
                    for name, value in stats[5]:
                        if "Downstream" in name:
                            name = "Downstream"
                        history[
                            "avgRef-" + name + "-" + armShorthandDescription(arm)
                        ].append((t, value))
                    history["pulls-" + armShorthandDescription(arm)].append((t, pulls))
                    history["avgLoss-" + armShorthandDescription(arm)].append(
                        (t, avgLoss)
                    )
                    history["avgObj-" + armShorthandDescription(arm)].append(
                        (t, avgObj)
                    )
                    history["avgCst-" + armShorthandDescription(arm)].append(
                        (t, avgCst)
                    )
        print("")
    return history
